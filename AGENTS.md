...
구현 로드맵은 [`docs/tasks/`](docs/tasks/)의 `TASK-##.md` 파일을 순서대로 따른다. 한 번에 하나의 TASK만 실행하고, 해당 TASK의 `Done Criteria`를 통과한 뒤 다음 TASK로 넘어간다. 아키텍처나 디자인 시스템 변경이 생기면 관련 TASK 문서와 `AGENTS.md`를 함께 갱신한다.
